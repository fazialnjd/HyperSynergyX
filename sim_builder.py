...
def build_chemical_similarity(drug_names, smiles_map):
    ...
    try:
        ...
        print("Warning: RDKit not installed. Returning identity matrix for chemical similarity.")
        ...
        for j, drug_b in enumerate(drug_names):
            if i == j:
                ...
                for level in range(3):
                    sim = jaccard_similarity(atc_by_level[drug_a][level], atc_by_level[drug_b][level])
        ...
        if data and len(data) > 0:
            ...
            if 'targets' in drug:
                ...
                if isinstance(targets, list):
                    result['targets'] = [t.get('name', t.get('id', '')) for t in targets if isinstance(t, dict)]
                else:
                    result['targets'] = []
    ...
    try:
        ...
        if response.status_code == 200 and 'drugbank.ca/drugs/' in response.text:
            ...
            if matches:
                ...
                # Extract SMILES
                smiles_match = re.search(r'SMILES["\s:]+([A-Za-z0-9\[\]()=@#$%\\/-]+)', drug_page.text)
                if smiles_match:
...
def load_drug_data_from_tsv(tsv_path, disease_type='breast'):
    ...
    if disease_type.lower() == 'breast':
        mask = df['Disease_Entry'].str.contains('breast|cancer', case=False, na=False) | \
               df['Cell_Line'].str.contains('MCF|ZR|T47D|MDAMB|SUM|BT', case=False, na=False)
    elif disease_type.lower() == 'lung':
        mask = df['Disease_Entry'].str.contains('lung|non-small|nsclc|sclc', case=False, na=False) | \
               df['Cell_Line'].str.contains('A549|H226|H1299|H322|H460|SK', case=False, na=False)
    else:
...
def build_breast(root):
    ...
    for drug in drug_names:
        ...
        drug_data = fetch_drug_from_drugbank(drug, cache_file=os.path.join(root, "drugbank_cache_breast.json"))
...
def build_lung(root):
    ...
    for drug in drug_names:
        print(f"  Fetching data for {drug}...")
        drug_data = fetch_drug_from_drugbank(drug, cache_file=os.path.join(root, "drugbank_cache_lung.json"))
        atc_map[drug] = drug_data['atc_codes']
        target_map[drug] = drug_data['targets']
